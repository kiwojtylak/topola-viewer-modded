...
var FancyChart = /** @class */ (function () {
    ...
    FancyChart.prototype.renderLeaves = function (nodes, svg) {
        ...
        gradient
            ...
            .attr('stop-color', '#8f8')
            .attr('stop-opacity', 0);
        const backgroundNodes = nodes.filter(function (n) {
            return n.parent && n.parent.id !== descendant_chart_1.DUMMY_ROOT_NODE_ID;
        });
        const minGeneration = d3_array_1.min(backgroundNodes, function (node) {
            return node.data.generation;
        }) || 0;
        const sizeFunction = function (node) {
        ...
        boundNodes = svg
            ...
            .data(backgroundNodes, function (d) {
                return d.id;
            });
        enter = boundNodes.enter().append('g');
        enter
            .merge(boundNodes)
        ...
        background = enter.append('g').attr('class', 'background');
        background
            .append('circle')
            .attr('class', 'background')
            .attr('r', sizeFunction)
        ...
        background = enter.append('g').attr('class', 'background2');
        background
            .append('circle')
            .attr('class', 'background')
            .attr('r', sizeFunction)
    ...
    FancyChart.prototype.render = function () {
        const nodes = descendant_chart_1.layOutDescendants(this.options, {
            flipVertically: true,
            vSpacing: 100,
        });
